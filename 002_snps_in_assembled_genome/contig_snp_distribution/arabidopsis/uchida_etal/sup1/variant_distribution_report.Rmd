...
mytheme <- theme(panel.background = element_rect(fill='white', colour='black'),
        ...
        plot.margin=unit(c(2,2,5,2),'mm'), legend.position=c(0.25,0.2), legend.title = element_blank(),
...
setwd("~/git-hub-repos/shyamrallapalli/mutations_hts_noref/002_snps_in_assembled_genome/contig_snp_distribution/arabidopsis/uchida_etal/sup2")
...
ggplot(data=tenkb, aes(x=position)) + stat_smooth(aes(y=ratio/sum(ratio), colour="homozygous to heterozygous ratio"),
  ...
  geom_vline(xintercept = as.numeric(80437067), size=0.5, linetype = "longdash") +
  annotate("text", x=0, y=0, label=lm_eqn(tenkb$position, tenkb$ratio, 12), hjust=0, size=8, family="Times", face="italic", parse=TRUE)
...
tenkb <- read.delim(file="per-5000000bp-filtered_sup1_colT_wsT.vcf.txt", header = TRUE)
tenkb$numhm <- tenkb$numhm + 0.1
tenkb$numht <- tenkb$numht + 0.1
tenkb$ratio <- tenkb$numhm/tenkb$numht
ggplot(data=tenkb, aes(x=position)) + stat_smooth(aes(y=ratio/sum(ratio), colour="homozygous to heterozygous ratio"),
...
tenkb <- read.delim(file="per-10000000bp-filtered_sup1_colT_wsT.vcf.txt", header = TRUE)
tenkb$numhm <- tenkb$numhm + 0.1
tenkb$numht <- tenkb$numht + 0.1
tenkb$ratio <- tenkb$numhm/tenkb$numht
ggplot(data=tenkb, aes(x=position)) + stat_smooth(aes(y=ratio/sum(ratio), colour="homozygous to heterozygous ratio"),
  ...
  scale_y_sqrt() + labs(y=NULL, main="genome in 10 KB fragment chunks") + mytheme +
  geom_vline(xintercept = as.numeric(80437067), size=0.5, linetype = "longdash") +
  annotate("text", x=0, y=0, label=lm_eqn(tenkb$position, tenkb$ratio, 8), hjust=0, size=8, family="Times", face="italic", parse=TRUE)
...
infile <- read.delim(file="varposn-genome-filtered_sup1_colT_wsT.vcf.txt", header = TRUE)
